...
def random_output(input_str: str, vectorizer, model) -> int:
    random_input = [input_str]
    random_input_vectorized = vectorizer.transform(random_input)
    val_preds_random = model.predict(random_input_vectorized)
    return val_preds_random[0]


vectorizer = None
model = None

# Have to add all of this to main because we don't want print statements to be executed in run_model
if __name__ == "__main__":
    ...
    test_df.to_csv(
        "../../data/splits/test_full.csv", index=False
    )  # Temporary full test set
